	...
	function auth ($perms = "read", $remember_uri = true)
	{
		...
		if (empty($_SESSION['phpFlickr_auth_token']) && empty($this->token)) {
			...
			if ($this->service == "23") {
				...
				header("Location: http://www.flickr.com/services/auth/?api_key=" . $this->api_key . "&extra=" . $redirect . "&perms=" . $perms . "&api_sig=". $api_sig,true);
				//return " http://www.flickr.com/services/auth/?api_key=" . $this->api_key . "&extra=" . $redirect . "&perms=" . $perms . "&api_sig=". $api_sig;
			}
		} else {
